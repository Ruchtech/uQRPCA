function [CQM] = cqm(orig_img, dist_img)
...
if (Z<3)
    ...
    for i=1:1:M*N
        ...
        % Y=(R+2*G+B)/4 
        YUV_img1(i)=floor((double(orig_img(i))+double(orig_img(M*N+i))*2+double(orig_img(2*M*N+i)))/4);   
        % U=R-G
        ...
        % Y=(R+2*G+B)/4 
        YUV_img2(i)=floor((double(dist_img(i))+double(dist_img(M*N+i))*2+double(dist_img(2*M*N+i)))/4);  
        % U=R-G
